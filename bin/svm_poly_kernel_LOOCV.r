...
cat("\014")
set.seed(12) # set.seed(12) for ZhangL


# #  Spira
# 
fileName <- "../data/Spira_GSE4115_GPL96_patients_data_filtered_ourSignature_transpose_3255.csv"
...
thisP <- 0.5
# execution_number <- 80 		# Philipsen signature on Heiskanen dataset, Philipsen signature on ZhangL dataset, 
execution_number <- 100

# TRAIN_SET_OVERSAMPLING_SYNTHETIC <- ROSE_OVERSAMPLE

# cat("fileName: ", fileName, "\n", sep="")
# cat("targetName: ", targetName, "\n", sep="")
cat("TRAIN_SET_OVERSAMPLING_SYNTHETIC: ", TRAIN_SET_OVERSAMPLING_SYNTHETIC, "\n", sep="")

list.of.packages <- c("easypackages", "e1071", "PRROC", "dplyr", "pastecs", "class", "gmodels", "kernlab", "ROSE", "klaR", "stringr")
...
# the column names should not start with a number
# colnames(patients_data) <- str_replace(colnames(patients_data), "^[0-9]", "P")
colnames(patients_data)[1:(ncol(patients_data)-1)] <- paste0("P", colnames(patients_data)[1:(ncol(patients_data)-1)])

for(i in 1:ncol(patients_data)) {

    patients_data[i] <- (as.double(unlist(patients_data[i])))

}

target_index <- dim(patients_data)[2]
all_predictions <- c()
all_labels <- patients_data[, target_index]

cat("Number of executions = ", nrow(patients_data), "\n", sep="")
# for(exe_i in 1:execution_number)
# exe_i <- 1
# while(exe_i <= execution_number)
for(w in 1:nrow(patients_data))
{
    cat("(w = ", w, ")\n", sep="")
    ...
    if(w == 1) {
                ...
                test_set_index <- w
                training_set_first_index <- 2
                training_set_last_index <- nrow(patients_data)
                    ...
                    patients_data_test_complete <- patients_data[test_set_index, 1:target_index] # NEW
                
                
    
    } else if(w == nrow(patients_data)) {
    
                test_set_index <- w
                training_set_first_index <- 1
                training_set_last_index <- w - 1
        ...
        training_set_first_part_index_START <- 1
        training_set_first_part_index_END <- w-1
        
        training_set_second_part_index_START <-  w+1
        training_set_second_part_index_END <- nrow(patients_data)
        
        
        cat("[Creating the training set and test set]\n")   
        patients_data_train_1st_part <- patients_data[training_set_first_part_index_START:training_set_first_part_index_END, ] 
        patients_data_train_2nd_part <- patients_data[training_set_second_part_index_START:training_set_second_part_index_END, ] 
        
        patients_data_test_complete <- rbind(patients_data_train_1st_part, patients_data_train_2nd_part)
        patients_data_test_complete <- patients_data[w,]
     ...
     if(TRAIN_SET_OVERSAMPLING_SYNTHETIC == TRUE)
         {
            ...
            data.rose <- ROSE(lung_cancer ~ ., data = patients_data_train_complete, p=thisP, seed = 1)$data
    ...
    if(imba_train_flag) {
	    ...
	    cat("\n\n\nSVM training completed\n\n\n")
            ...
            # easy fix for NAs, but I have to improve it
            indexNAs <- which(is.na(patients_data_test_pred$posterior[,2]))
            if( length(indexNAs) >=1) {
                patients_data_test_pred$posterior[indexNAs,2] <- abs((patients_data_test_pred$posterior[indexNAs,1])[[1]] -1)
            }
            
                        
            patients_data_test_binary_prediction <- as.numeric((patients_data_test_pred$posterior[,2])>=threshold)
            
            
            all_predictions[w] <- patients_data_test_binary_prediction
 ...
 cat("\n\n\n=== final results ===\n")
  ...
  thisConfMat <- confusion_matrix_rates(all_labels, all_predictions, "@@@ Test set @@@")
...
# meanRowResults <- (statDescConfMatr)[c("mean", "std.dev"),]
# cat("\n\n")
