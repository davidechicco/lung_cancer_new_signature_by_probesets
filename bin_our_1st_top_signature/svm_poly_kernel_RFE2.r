...
cat("\014")
set.seed(18) # set.seed(12) for ZhangL
...
predictionProbesetsTable <- list(probeset=character(), predictionMCC=numeric())


# RFE loop
for(w in 1:(ncol(patients_data_original)-1)) {

	patients_data <- patients_data_original
	thisProbesetRemoved <- colnames(patients_data)[w]

	cat("\n\n### probeset removed here: ", thisProbesetRemoved, ", that is the ", w, " column\n", sep="")
	# patients_data[, w] <- NULL
	patients_data <- patients_data[,!(names(patients_data) %in% thisProbesetRemoved)]
	...
	while(exe_i <= execution_number)
	{
	    ...
	    if(imba_train_flag & imba_test_flag) {
		    ...
		    # svm_model_new <- svm(patients_data_train_labels ~ ., cost=fixed_c, data=patients_data_train, method = "C-classification", kernel = "polynomial", degree=2)
		   ...
		   print(patients_data_test_pred)
		    ...
		    # patients_data_test_binary_predictions <- patients_data_test_pred
	...
	cat("\n\n\n=== ", thisProbesetRemoved, " final results ===\n")
	...
	predictionProbesetsTable$"probeset"[w] <- thisProbesetRemoved
	predictionProbesetsTable$"predictionMCC"[w] <- meanRowResults[[1]]
}

cat("\n\n=== === === ===\n")
cat("@probeset predictionMCC ", datasetName,"\n")
cat("- - - - - - - - - - - - - - - - - - \n")
for(h in 1:length(predictionProbesetsTable$probeset)) {

  cat(predictionProbesetsTable$probeset[h], " ", predictionProbesetsTable$predictionMCC[h], " \n", sep="")
