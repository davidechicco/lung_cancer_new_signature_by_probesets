...
LABEL_DETECTED <- TRUE
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    # # # # we add the labels
    library("plyr")
    label_list <- as.numeric(revalue(gset@phenoData@data$"history:ch1", c("current smoker without cancer"=0, "former smoker without cancer"=0,"never smoker"=0, "current smoker with cancer"=1,"former smoker with cancer"=1)))
...
#      RotunnoSignature <- c(" 204198_s_at", "204731_at", "215806_x_at", "209813_x_at", "211144_x_at", "216920_s_at", "215227_x_at", "211571_s_at", "215646_s_at", "36936_at")
     ...
     PhilipsenSignature <- c("210081_at", "206209_s_at", "209074_s_at", "238222_at", "201291_s_at")
    
    patients_data_filtered_ourSignature <- (gene_expression_with_labels[gene_expression_with_labels$ID %in% PhilipsenSignature, ])
